...
def JonesVdeltaPhi (JonesV):
    ...
    return np.array([[np.angle(JonesV[1][0])],[np.arccos(JonesV[0][0])]])

def StokesVdeltaPhi (StokesV):

     #takes a stokes vector object and returns a 2x1 array with delta on the first row and phi on the bottom.
    delta_list = []
    phi_list = []
    ...
    return np.array([[delta_list[0]],[phi_list[0]]])

def JonesVtoStokesV (JonesV):

    deltaphi = JonesVdeltaPhi(JonesV)

    return np.array([[1],[np.cos(2*deltaphi[1][0])],[np.sin(2*deltaphi[1][0])*np.cos(deltaphi[0][0])],
                     [-np.sin(2*deltaphi[1][0])*np.sin(deltaphi[0][0])]])

def StokesVtoJonesV (StokesV):

    deltaphi = StokesVdeltaPhi(StokesV)

    return np.array([[np.cos(deltaphi[1][0])], [np.conjugate(np.exp(deltaphi[0][0]*1j)*np.sin(deltaphi[1][0]))]])
